...
class ImagePuzzle:
    ...
    def __init__(self, grid_size, tile_size, margin_size):
        ...
        self.tilespos = {(x, y): (x * (tile_size + margin_size) + margin_size, y * (tile_size + margin_size) + margin_size)
                         for y in range(grid_size[1]) for x in range(grid_size[0])}
        w, h = grid_size[0] * (tile_size + margin_size) + margin_size, grid_size[1] * (tile_size + margin_size) + margin_size

        image_path = 'C:\\Users\\royal\\Downloads\\d2601da98d72c463b33ddd9af3a34a49.jpg'
        if not os.path.exists(image_path):
            print(f"Image file not found: {image_path}")
            pygame.quit()
            sys.exit()

        pic = pygame.image.load(image_path)
...
def main():
    ...
    if not os.path.exists(image_path):
        ...
        pygame.quit()
        sys.exit()
    img1 = pygame.image.load(image_path)  # loading the image once again to a separate variable
